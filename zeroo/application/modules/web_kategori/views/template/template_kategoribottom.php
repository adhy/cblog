...
function category(){                   
		...
		if(loadkategori.readyState==0||loadkategori.readyState==4){
			var urll = "<?php echo site_url('parse/json/view/home/category/'.$link.''); ?>";
		...
		if(loadkategori.readyState==4 ){
			...
			var view = JSON.parse(loadkategori.responseText);
				...
				for(i=0;i<view.length;i++){
					...
					out +='<p><i class="fa fa-user"></i> by <a href="<?php echo site_url("about"); ?>"> <span id="by">'+view[i].by_me+'</span>  </a> | <i class="fa fa-folder"></i> Category <a href="#'+view[i].name_c+'"> '+view[i].name_c+' </a> | <i class="fa fa-calendar"></i> '+view[i].date_post+'</p><hr>';
				...
				document.getElementById("category").innerHTML=out;
						...
						document.getElementById("category").innerHTML=not_found;
			}
		}
	}
